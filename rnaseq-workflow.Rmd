...
```{r}
samples <- read_csv("/home/ytfang/2026-ggg-201b-hw3-ytfang-ytf/rnaseq_samples.csv")
```
...
**A:** 
Lots of the low mean expression genes, most of them may potentially be the noise with low statistic power.
...
**A:** 
gene *YAL005C* as the most significant gene that show significant different. (i.e., the lowest adjusted p-value)
...
```{r}
mds$condition <- factor(mds$condition , levels = c("wt", "snf2")) 

ggplot(mds, aes(X1, X2, shape = condition, color = condition)) + 
  geom_point(size = 3) +
  theme_minimal()
``` 
...
**A:**
Different condition can be separate by the dimension 1. However, *wt* show more variation in dimension 2 compare to *snf2*.
...
**A:**
Heatmaps are useful when want to visualize patterns across samples and genes; Heatmaps can be also found when observing the correlation between different features. e.g., the correlation between *module eigenegene* and the *trait/condition/...* in WGCNA analysis. 
